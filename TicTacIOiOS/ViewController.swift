...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        socket = SocketIOClient(socketURL: URL(string: urlStr)!, config: [.log(true), .path("/chat/socket.io/"), .connectParams(["token":jwt]), .secure(true)])
        ...
        socket?.on("messages.send.6b9280a7-8af0-4d74-be89-d01fdcd39afc") {data, ack in
